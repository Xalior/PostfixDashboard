...
/**
 * Password hashing that speaks Dovecot's `{SCHEME}hash` format.
 *
 ...
 * BLF-CRYPT, which is bcrypt and something Dovecot verifies natively).
 *
 * Verify supports:
...
export async function hashPassword(plain: string): Promise<string> {
  const scheme = env.password.scheme.toUpperCase() as PasswordScheme;
  switch (scheme) {
    case 'BLF-CRYPT': {
      const h = await bcrypt.hash(plain, env.password.bcryptRounds);
      return `{BLF-CRYPT}${h}`;
    }
    case 'PLAIN':
      return `{PLAIN}${plain}`;
...
export async function verifyPassword(
  ...
  try {
    switch (scheme) {
      ...
      case 'MD5-CRYPT':
        // $1$ MD5-crypt is too legacy to re-implement safely here.
        // Detect & reject so users know to migrate.
        return false;
 ...
 */
function verifySha512Crypt(plain: string, stored: string): boolean {
